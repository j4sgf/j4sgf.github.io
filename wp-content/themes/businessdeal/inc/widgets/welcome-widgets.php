 ...
 */
 ...
 class BusinessDeal_welcome extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'widget_welcome_section', 'description' => esc_html__( 'Display  Welcome Section in home page', 'businessdeal') );
		$control_ops = array('width' => 200, 'height' => 250);
		parent::__construct( false, $name=esc_html__('T-Spiral: Welcome Section','businessdeal'), $widget_ops, $control_ops );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'select_layout' ) ); ?>">
				<?php esc_html_e( 'Select Welcome Layout:', 'businessdeal' ); ?>
			</label>
			<br/>
	...
	public function widget( $args, $instance ) {
		...
		for( $i=0; $i<8; $i++ ) {
 		...
 		if( !empty($page_array) ) {
	 			...
	 			<?php
	 				while( $get_featured_pages->have_posts() ):$get_featured_pages->the_post(); ?>
	 					<div class="column ch-column">
							<div class="welcome-item-inside">
								<?php if ( has_post_thumbnail() ) { ?>
									<div class="wi-thumbnail">
										<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
											<?php the_post_thumbnail('businessdeal-welcome-widget'); ?>
										</a>
									</div><!-- .wi-thumbnail -->
								<?php } ?>
								<div class="wi-content">
									<div class="wi-content-holder">
										<h2 class="wi-title"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
										<div class="wi-text">
											<?php the_excerpt(); ?>
										</div><!-- .wi-text -->
									</div><!-- .wi-content-holder -->
								</div><!-- .wi-content -->
			...
			<?php echo $after_widget. '<!-- .widget_welcome_section -->';
		}
	}
}
